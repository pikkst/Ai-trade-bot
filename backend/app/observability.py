...
_SENSITIVE_KEY_RE = re.compile(
    r"(^|_)(authorization|cookie|password|passwd|secret|token|api_key|apikey|database_url|dsn)($|_)",
    re.IGNORECASE,
)
_BEARER_RE = re.compile(r"(?i)\bbearer\s+[A-Za-z0-9._~+/=-]+")
_URL_CREDENTIAL_RE = re.compile(r"(?P<scheme>[a-zA-Z][a-zA-Z0-9+.-]*://)[^\s/@:]+:[^\s/@]+@")


def redact_value(value: Any, *, key: str | None = None) -> Any:
    """Recursively redact credentials while preserving operational identifiers."""
