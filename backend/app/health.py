...
def liveness() -> HealthStatus:
    """Prove the process can serve Python code without external I/O."""
...
def readiness(
    *,
    ...
    """Verify required dependencies without exposing exception details."""
    checks = {"process": "ok"}
    ...
    if factory is None:
        raise ValueError(
            "a session factory is required when database readiness is enabled"
        )
    try:
