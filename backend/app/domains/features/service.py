...
class FeatureService:
    ...
    def compute_features(self) -> FeatureResult:
        ...
        idempotency_key = self._build_idempotency_key(snapshot, feature_set, input_hash)
        ...
        try:
            feature_values.extend(self._compute_returns(candles, warnings))
            feature_values.extend(self._compute_sma(candles, warnings))
            feature_values.extend(self._compute_ema(candles, warnings))
            feature_values.extend(self._compute_rsi(candles, warnings))
            feature_values.extend(self._compute_atr(candles, warnings))
            feature_values.extend(self._compute_volatility(candles, warnings))
            feature_values.extend(self._compute_volume_relative(candles, warnings))
        ...
        output_hash = self._compute_output_hash(feature_values)
        ...
        self._insert_values(calculation.id, feature_values)
        return FeatureResult(calculation=calculation, values=tuple(feature_values))
    ...
    def _load_snapshot(self) -> dict[str, Any]:
        ...
        if snapshot["state"] != "active":
            raise ValueError(
                f"snapshot {self._snapshot_id} state={snapshot['state']} is not active"
            )
    ...
    def _load_snapshot_candles(self, snapshot: dict[str, Any]) -> list[CandleData]:
        rows = (
            self._session.execute(
                text(
                    """
    ...
    def _insert_values(self, calculation_id: UUID, values: list[FeatureValue]) -> None:
        if not values:
            return
        value_rows = []
        ...
        sql = (
            "insert into public.feature_values "  # nosec B608: parameterized values
        ...
        if len(candles) >= ema_50_period:
            ...
            for i in range(len(candles)):
                ...
                if i == ema_50_period - 1:
                    ...
                    ema_50 = (
                        alpha_50 * candles[i].close + (Decimal("1") - alpha_50) * ema_50
                    )
                ema_50_values.append(
                    self._make_value(
                        FeatureCode.EMA_50,
    ...
    def _compute_volume_relative(
        ...
        if len(candles) < period:
            for i in range(len(candles)):
                values.append(
                    self._make_value(
                        FeatureCode.VOLUME_RELATIVE_20,
                        i,
                        candles[i].open_time,
                        null_reason=_WARM_UP_NULL_REASON,
                        unit="ratio",
