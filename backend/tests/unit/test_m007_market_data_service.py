...
@pytest.mark.asyncio
async def test_incremental_fetch_no_new_data(database_engine: Engine) -> None:
    provider = FakeBinanceProvider(
        config=FakeBinanceConfig(
            scenario=FakeBinanceScenario.SUCCESS,
            fixed_clock_time=FIXED_TIME,
...
@pytest.mark.asyncio
async def test_detect_gaps_finds_missing(database_engine: Engine) -> None:
    provider = FakeBinanceProvider(
        config=FakeBinanceConfig(
            scenario=FakeBinanceScenario.SUCCESS,
            fixed_clock_time=FIXED_TIME,
...
def test_create_snapshot(database_engine: Engine) -> None:
    provider = FakeBinanceProvider(
        config=FakeBinanceConfig(
            scenario=FakeBinanceScenario.SUCCESS,
            fixed_clock_time=FIXED_TIME,
...
@pytest.mark.asyncio
async def test_idempotent_backfill_returns_existing(database_engine: Engine) -> None:
    provider = FakeBinanceProvider(
        config=FakeBinanceConfig(
            scenario=FakeBinanceScenario.SUCCESS,
            fixed_clock_time=FIXED_TIME,
