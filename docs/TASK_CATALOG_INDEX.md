...
This index preserves the repository’s detailed UX, cloud, local/production, and Sprint task cards while removing ambiguity about implementation order.
...
- **Superseded** — the original implementation assumption is replaced by the active architecture, but useful acceptance criteria may be retained;
- **Future assessment** — outside M001–M036 and requires a separate approved milestone;
- **Documentation complete** — specification exists, but implementation remains not started until its Master Task is selected and verified.

## 3. Conflict Resolution

When a detailed task’s dependency, priority, or architecture conflicts with `TASKS.md`:

1. keep the detailed acceptance criteria that remain compatible;
2. use the Master Task dependency and active architecture;
3. mark the conflicting implementation assumption superseded or deferred;
...
- a historical task requiring Redis/ARQ is superseded by the one-shot CLI and PostgreSQL lock model;
- a historical WebSocket task is deferred; REST finalized-candle ingestion is mandatory;
- local Supabase work is M003 and does not depend on cloud project creation;
- hosted Prometheus/Grafana is deferred; persistent cycle/audit/freshness/halt/reconciliation evidence is mandatory;
- private Binance, Binance test execution, and live trading are future assessments.

## 4. Root and Governance Catalogs
...
| `SPRINT_21_TASKS.md` | documentation governance | In progress | Aligns catalog and lifecycle references |

## 5. Legacy Root Backlog Mapping

The former root `T*` tasks are retained only in Git history and references inside Master Tasks.
...
| local Docker stack requiring Redis/worker/Prometheus/Grafana | M001–M003 | Superseded; only local Supabase/application needs remain |
| GitHub quality workflow | M002 | Mandatory |
| settings, errors, logging, transactions, idempotency | M005 | Mandatory |
| database/migration baseline | M003 | Mandatory |
...
| Gemini provider, schema, validation, budget | M009 | Mandatory |
...
| hosted Prometheus/Grafana | future change through M034 | Deferred |

## 6. UX Design Catalog Mapping

| Detailed IDs / area | Master Task | Classification |
|---|---|---|
...
## 7. Cloud MVP Catalog Mapping

| Detailed ID | Master Task | Classification | Canonical dependency note |
|---|---|---|---|
| C1 dedicated Supabase cloud project | M028 | Mandatory | follows M026–M027; not required for local work |
| C2 cloud migrations, Auth, RLS | M028 | Mandatory | local schema/RLS foundation comes from M003 |
| C3 one-shot research-cycle CLI | M012 | Mandatory | implemented locally before cloud deployment |
| C4 scheduled GitHub Actions cycle | M028 | Mandatory | depends on verified M012 and M027 |
...
| C7 free-tier observability and backup procedure | M029 | Mandatory | restore foundation from M027 |
...
- delayed or missed cycles use actual eligible market data and never create imagined trades;
...
## 8. Local and Production Catalog Mapping
...
| L2.1 unit/property tests | M002 and M026 | Mandatory |
| L2.2 migration/constraint/Auth/RLS tests | M003 and M026 | Mandatory |
...
| `SPRINT_16_TASKS.md` — Data Lifecycle/Dataset Governance | M031 | Requires implemented data domains and experiment evidence |
...
| `SPRINT_20_TASKS.md` — Documentation Synchronization | documentation governance | Completed and verified |
| `SPRINT_21_TASKS.md` — Catalog/Lifecycle Synchronization | documentation governance | In progress |
...
## 10. Environment Lifecycle Mapping
...
The following remain excluded from mandatory M001–M036 implementation unless a later Master Task change proposal activates them:
...
Any activation requires M034-style proposal, behavior diff, impact, security/privacy, migration, cost/capacity, testing, staged paper verification, rollback, and owner approval. Exchange credential or live-capital changes additionally require a separate milestone outside this index.

## 12. Developer Selection Procedure
...
5. Select the exact mandatory and applicable conditional cards.
6. Treat deferred/superseded assumptions as excluded.
7. Implement, test, document, and record evidence.
8. Mark the Master Task `VERIFIED` only after all required evidence exists.

## 13. Completion Rule

Detailed-card completion is necessary but not sufficient. A Master Task is complete only when:

- all mandatory detailed cards are implemented or approved not applicable;
