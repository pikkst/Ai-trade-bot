...
alter table public.exchange_symbol_versions
    add column if not exists raw_metadata_hash text not null check (length(raw_metadata_hash) = 64);

alter table public.exchange_symbol_versions
    add column if not exists retrieved_at timestamptz not null default timezone('utc', now());
